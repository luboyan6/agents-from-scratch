...
该模块实现了 ReAct (Reasoning and Acting) 范式的 Agent，
通过交替进行推理（Thought）和行动（Action）来解决问题。
...
class ReActAgent(Agent):
    ...
    def __init__(
        ...
        Args:
            name: Agent 名称
            llm: LLM 客户端实例
            tool_registry: 工具注册表（可选，不提供则创建空的注册表）
        ...
        self.current_history: List[str] = []                           # 当前执行历史
        self.prompt_template = custom_prompt if custom_prompt else DEFAULT_REACT_PROMPT  # 提示词模板

    # ==================== 公有方法 ====================

    def add_tool(self, tool):
        """
        添加工具到工具注册表，支持 MCP 工具的自动展开
