...
class QdrantConnectionManager:
    ...
    def get_instance(
        ...
        collection_name: str = "open_agents_vectors",
        vector_size: int = 384,
        distance: str = "cosine",
        timeout: int = 30,
        **kwargs
...
class QdrantVectorStore:
    ...
    成员变量:
        ...
        timeout: 连接超时时间
        hnsw_m: HNSW的M参数
        hnsw_ef_construct: HNSW的ef_construct参数
        search_ef: 搜索时的ef参数
        search_exact: 是否使用精确搜索
        distance: 距离度量方式
        client: Qdrant客户端
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # ==================== 成员变量 ====================
        self.url = url                                        # Qdrant服务URL
        self.api_key = api_key                                # API密钥
        self.collection_name = collection_name                # 集合名称
        self.vector_size = vector_size                        # 向量维度
        self.timeout = timeout                                # 连接超时时间
        # HNSW/Query params via env
        try:
            self.hnsw_m = int(os.getenv("QDRANT_HNSW_M", "32"))  # HNSW的M参数
        except Exception:
            self.hnsw_m = 32
        try:
            self.hnsw_ef_construct = int(os.getenv("QDRANT_HNSW_EF_CONSTRUCT", "256"))  # HNSW的ef_construct参数
        except Exception:
            self.hnsw_ef_construct = 256
        try:
            self.search_ef = int(os.getenv("QDRANT_SEARCH_EF", "128"))  # 搜索时的ef参数
        except Exception:
            self.search_ef = 128
        self.search_exact = os.getenv("QDRANT_SEARCH_EXACT", "0") == "1"  # 是否使用精确搜索
        ...
        self.distance = distance_map.get(distance.lower(), Distance.COSINE)  # 距离度量方式

        # 初始化客户端
        self.client = None                                    # Qdrant客户端
