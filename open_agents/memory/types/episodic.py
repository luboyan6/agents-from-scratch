...
class EpisodicMemory(BaseMemory):
    ...
    成员变量:
        episodes: 情景列表（内存缓存）
        sessions: 会话到情景ID的映射字典
        patterns_cache: 模式识别缓存
        last_pattern_analysis: 上次模式分析时间
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # ==================== 成员变量 ====================
        # 本地缓存（内存）
        self.episodes: List[Episode] = []                 # 情景列表
        self.sessions: Dict[str, List[str]] = {}          # 会话到情景ID的映射

        # 模式识别缓存
        self.patterns_cache = {}                          # 模式识别缓存
        ...
        # 统一嵌入模型（多语言，默认384维）
        self.embedder = get_text_embedder()               # 文本嵌入模型

        # 向量存储（Qdrant - 使用连接管理器避免重复连接）
        from ..storage.qdrant_store import QdrantConnectionManager
        qdrant_url = os.getenv("QDRANT_URL")
        qdrant_api_key = os.getenv("QDRANT_API_KEY")
        self.vector_store = QdrantConnectionManager.get_instance(  # Qdrant向量存储
            url=qdrant_url,
