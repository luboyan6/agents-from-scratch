...
class WorkingMemory(BaseMemory):
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        Args:
            config: 记忆配置对象
            storage_backend: 存储后端（工作记忆通常不需要）
        """
        super().__init__(config, storage_backend)

        # ==================== 成员变量 ====================
        # 工作记忆特定配置
        self.max_capacity = self.config.working_memory_capacity     # 最大容量
        self.max_tokens = self.config.working_memory_tokens         # 最大token数
        # 纯内存TTL（分钟），可通过在 MemoryConfig 上挂载 working_memory_ttl_minutes 覆盖
        self.max_age_minutes = getattr(self.config, 'working_memory_ttl_minutes', 120)  # TTL
        self.current_tokens = 0                                      # 当前token数
        self.session_start = datetime.now()                         # 会话开始时间

        # 内存存储（工作记忆不需要持久化）
        self.memories: List[MemoryItem] = []                        # 内存存储列表

        # 使用优先级队列管理记忆
        self.memory_heap = []                                        # 优先级队列 (priority, timestamp, memory_item)
