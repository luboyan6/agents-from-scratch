...
该模块实现了基础的对话 Agent，支持：
- 简单对话交互
- 可选的工具调用（通过文本标记解析）
- 流式响应
- 消息历史管理

这是最基础的 Agent 实现，其他 Agent 都在此基础上扩展。
"""
...
class SimpleAgent(Agent):
    ...
    工具调用通过文本标记 `[TOOL_CALL:tool_name:parameters]` 实现，
    与 FunctionCallAgent 的原生函数调用不同。
    ...
    def __init__(
        ...
        Args:
            ...
            config: 配置对象（可选）
            tool_registry: 工具注册表（可选，提供则启用工具调用）
            enable_tool_calling: 是否启用工具调用（需要提供 tool_registry）
        """
    ...
    def run(self, input_text: str, max_tool_iterations: int = 3, **kwargs) -> str:
        """
        运行 SimpleAgent，支持可选的工具调用（重写父类方法）
