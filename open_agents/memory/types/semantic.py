...
class Entity:
    ...
    成员变量:
        entity_id: 实体唯一标识
        name: 实体名称
        entity_type: 实体类型（PERSON/ORG/PRODUCT/SKILL/CONCEPT等）
        description: 实体描述
        properties: 实体属性字典
        created_at: 创建时间
        updated_at: 更新时间
    ...
    def __init__(
        ...
        self.properties = properties or {}               # 实体属性字典
...
class Relation:
    ...
    def __init__(
        ...
        # ==================== 成员变量 ====================
        self.from_entity = from_entity                   # 源实体ID
        self.to_entity = to_entity                       # 目标实体ID
        self.relation_type = relation_type               # 关系类型
        self.strength = strength                         # 关系强度
        self.evidence = evidence                         # 支持该关系的原文本
        self.properties = properties or {}               # 关系属性字典
        self.created_at = datetime.now()                 # 创建时间
...
class SemanticMemory(BaseMemory):
    ...
    成员变量:
        ...
        graph_store: Neo4j图存储
        entities: 实体缓存字典
        relations: 关系列表
        nlp: NLP处理器
        nlp_models: 多语言NLP模型字典
        semantic_memories: 语义记忆列表
        memory_embeddings: 记忆ID到嵌入向量的映射
    """
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # ==================== 成员变量 ====================
        # 嵌入模型（统一提供）
        self.embedding_model = None                       # 文本嵌入模型
        self._init_embedding_model()

        # 专业数据库存储
        self.vector_store = None                          # Qdrant向量存储
        self.graph_store = None                           # Neo4j图存储
        self._init_databases()

        # 实体和关系缓存 (用于快速访问)
        self.entities: Dict[str, Entity] = {}             # 实体缓存字典
        self.relations: List[Relation] = []               # 关系列表

        # 实体识别器
        self.nlp = None                                   # NLP处理器
        self.nlp_models = {}                              # 多语言NLP模型字典
        self._init_nlp()

        # 记忆存储
        self.semantic_memories: List[MemoryItem] = []     # 语义记忆列表
        self.memory_embeddings: Dict[str, np.ndarray] = {}  # 记忆嵌入向量映射
