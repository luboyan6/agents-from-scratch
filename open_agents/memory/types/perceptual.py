...
class Perception:
    ...
    用于封装单个感知数据单元，支持多模态数据。

    继承:
        无

    成员变量:
        perception_id: 感知数据唯一标识
        data: 感知数据内容
        modality: 模态类型（text/image/audio/video/structured）
        encoding: 编码向量
        metadata: 元数据字典
        timestamp: 创建时间戳
        data_hash: 数据哈希值
    ...
    def __init__(
        ...
        # ==================== 成员变量 ====================
        self.perception_id = perception_id               # 感知数据唯一标识
        self.data = data                                 # 感知数据内容
        self.modality = modality                         # 模态类型
        self.encoding = encoding or []                   # 编码向量
        self.metadata = metadata or {}                   # 元数据字典
        self.timestamp = datetime.now()                  # 创建时间戳
        self.data_hash = self._calculate_hash()          # 数据哈希值
...
class PerceptualMemory(BaseMemory):
    ...
    成员变量:
        perceptions: 感知数据缓存字典
        perceptual_memories: 感知记忆列表
        modality_index: 模态索引字典
        supported_modalities: 支持的模态集合
        doc_store: SQLite文档存储
        text_embedder: 文本嵌入模型
        vector_dim: 向量维度
        _clip_model: CLIP图像模型
        _clip_processor: CLIP处理器
        _clap_model: CLAP音频模型
        _clap_processor: CLAP处理器
        _image_dim: 图像向量维度
        _audio_dim: 音频向量维度
        vector_stores: 按模态拆分的向量存储字典
        encoders: 编码器字典
    """
    ...
    def __init__(self, config: MemoryConfig, storage_backend=None):
        ...
        # ==================== 成员变量 ====================
        # 感知数据存储（内存缓存）
        self.perceptions: Dict[str, Perception] = {}      # 感知数据缓存字典
        self.perceptual_memories: List[MemoryItem] = []   # 感知记忆列表

        # 模态索引
        self.modality_index: Dict[str, List[str]] = {}   # 模态索引字典

        # 支持的模态
        self.supported_modalities = set(self.config.perceptual_memory_modalities)  # 支持的模态集合

        # 文档权威存储（SQLite）
        db_dir = getattr(self.config, 'storage_path', "./memory_data")
        os.makedirs(db_dir, exist_ok=True)
        db_path = os.path.join(db_dir, "memory.db")
        self.doc_store = SQLiteDocumentStore(db_path=db_path)  # SQLite文档存储

        # 嵌入维度（与统一文本嵌入保持一致）
        self.text_embedder = get_text_embedder()          # 文本嵌入模型
        self.vector_dim = get_dimension(getattr(self.text_embedder, 'dimension', 384))  # 向量维度

        # 可选加载：图像CLIP与音频CLAP（缺依赖则优雅降级为哈希编码）
        self._clip_model = None                           # CLIP图像模型
        self._clip_processor = None                       # CLIP处理器
        self._clap_model = None                           # CLAP音频模型
        self._clap_processor = None                       # CLAP处理器
        self._image_dim = None                            # 图像向量维度
        self._audio_dim = None                            # 音频向量维度
        try:
        ...
        base_collection = os.getenv("QDRANT_COLLECTION", "open_agents_vectors")
        ...
        self.encoders = self._init_encoders()             # 编码器字典
