"""
OpenAgents 统一 LLM 接口模块

基于 OpenAI 原生 API，提供统一的 LLM 调用接口。
支持多种 LLM 提供商，包括 OpenAI、DeepSeek、Qwen、Kimi、智谱、Ollama 等。
...
# 支持的 LLM 提供商类型（使用 Literal 进行类型约束）
...
class OpenAgentsLLM:
    ...
    为 OpenAgents 定制的 LLM 客户端，调用任何兼容 OpenAI 接口的服务。
    默认使用流式响应以提供更好的用户体验。
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            model: 模型名称，未提供则从 LLM_MODEL_ID 环境变量读取
            api_key: API 密钥，未提供则从环境变量读取
            base_url: 服务地址，未提供则从 LLM_BASE_URL 环境变量读取
            provider: LLM 提供商，未提供则自动检测
        ...
        if not all([self.api_key, self.base_url]):
            raise OpenAgentsException("API密钥和服务地址必须被提供或在.env文件中定义。")
    ...
    def think(self, messages: list[dict[str, str]], temperature: Optional[float] = None) -> Iterator[str]:
        ...
        Args:
            ...
            temperature: 温度参数，未提供则使用初始化时的值

        Yields:
            str: 流式响应的文本片段

        Raises:
            OpenAgentsException: LLM 调用失败时抛出
        """
        ...
        try:
            ...
            raise OpenAgentsException(f"LLM调用失败: {str(e)}")

    def invoke(self, messages: list[dict[str, str]], **kwargs) -> str:
        """
        非流式调用 LLM

        适用于不需要流式输出的场景，返回完整响应。
